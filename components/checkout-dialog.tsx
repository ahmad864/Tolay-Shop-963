...
import { generateOwnerWhatsAppMessage, type CustomerInfo } from "@/lib/whatsapp-utils"
...
export function CheckoutDialog({ open, onOpenChange }: CheckoutDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const storeNumber = "+963987654321" // رقم المتجر السوري
      const message = generateOwnerWhatsAppMessage(state.items, customerInfo)
      const whatsappUrl = `https://wa.me/${storeNumber}?text=${message}`

      // Send order silently in background
      window.open(whatsappUrl, "_blank")

      // Show success message to customer
      alert("تم إرسال طلبك بنجاح! سنتواصل معك قريباً لتأكيد التفاصيل.")
                  ...
                  <SelectContent>
                    {syrianCities.map((city) => (
                      <SelectItem key={city.name} value={city.name}>
                        {city.name} - رسوم الشحن: ${city.shipping}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
